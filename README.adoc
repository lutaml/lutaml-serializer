...
It groups a set of element declarations, allowing them to be reused together.
Note: It is xml only. Overwriting of elements and attributes depends on the hierarchy of importing the model.

Group is used in Lutaml by defining a `no-root` class and `import_model`, `import_model_attributes` and `import_model_mappings` in another class.
...
Choice supports (element | choice)*
...
Sequence supports (element | sequence)*
...
* `sequence` :: Validates that elements appear in specified order in xml
...
#      #<Lutaml::Model::OutOfRangeChoiceError: Attributes must be in specified range in a choice>
#    ]
