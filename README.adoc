...
Group supports (choice | sequence | all)
...
.Using the `group` to define a collection of attributes
[example]
====
[source,ruby]
----
...
The Choice ensures that exactly one element from the specified group is included.

Choice supports (element | group | choice)*

.Using the `choice` to define a set of attributes where exactly one can be used.
[example]
====
[source,ruby]
----
...
The sequence ensures that the given elements must be in specified order.
...
=== Attributes within All

The all ensures that all elements must appear in any order.

All supports (element)*. It can only appear within a group and only contain elements.

.Using the `all` to define a set of elements to appear in any order.
[example]
====
[source,ruby]
----
...
* `all` :: Validates that all elements must appear in any order
...
klin = Klin.new(name: "Klin", degree_settings: [100, 200, 300], description: "one", prefix: "Ben", id: 1, age: 24, lname: "Azam", fname: "Babar")
klin.validate
# => []

klin = Klin.new(name: "Klin", degree_settings: [], description: "four", prefix: "Ben", nick_name: "Smith", age: 24, id: 1, fname: "Saim")
klin.validate
# => [
...
#      #<Lutaml::Model::InvalidAllError: All attributes must appear in any order>
#    ]
...
#      All attributes must appear in any order
#    ]
e.errors
# => [
#     #<Lutaml::Model::CollectionSizeError: degree_settings must have at least 1 element>,
